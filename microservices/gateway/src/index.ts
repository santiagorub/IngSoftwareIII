...
type PendingMap = Map<string, { resolve: (v: any) => void; reject: (e: any) => void; timeout: NodeJS.Timeout }>; 
...
app.post('/cart/add', async (req: Request, res: Response) => {
  ...
  const event = { type: 'cart.add.request', payload: { userId, productId, quantity }, correlationId, ts: Date.now() };
  logger.info({ event }, 'Publishing add to cart request');

  try {
    await producer.send({ topic: 'cart.add.request', messages: [{ key: correlationId, value: JSON.stringify(event) }] });
  } catch (err) {
