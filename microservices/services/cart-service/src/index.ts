...
const kafka = new Kafka({ clientId: 'cart-service', brokers: KAFKA_BROKERS, logLevel: logLevel.INFO });
...
type ValidationState = { user?: 'success' | 'failure'; product?: 'success' | 'failure'; reason?: string };
const state: Map<string, ValidationState> = new Map();

let dbFailures = 0;
let circuitOpenUntil = 0;

function dbCircuitAvailable(): boolean { return Date.now() >= circuitOpenUntil; }
function reportDbFailure() { dbFailures++; if (dbFailures >= 3) { circuitOpenUntil = Date.now() + 10000; logger.warn({ circuitOpenUntil }, 'Cart DB circuit opened'); dbFailures = 0; } }
function reportDbSuccess() { dbFailures = 0; }

async function tryCompleteSaga(correlationId: string, payload: { userId: number; productId: string; quantity: number }) {
  ...
  if (s.user === 'success' && s.product === 'success') {
    if (!dbCircuitAvailable()) {
      await producer.send({ topic: 'cart.add.result', messages: [{ key: correlationId, value: JSON.stringify({ correlationId, status: 'failure', reason: 'cart_db_circuit_open' }) }] });
      state.delete(correlationId);
      return;
    }
    try {
      await prisma.cartItem.create({ data: { userId: payload.userId, productId: payload.productId, quantity: payload.quantity } });
      reportDbSuccess();
      await producer.send({ topic: 'cart.add.result', messages: [{ key: correlationId, value: JSON.stringify({ correlationId, status: 'success' }) }] });
      logger.info({ correlationId }, 'Cart item created, saga success');
    } catch (err) {
      logger.error({ err }, 'Cart DB error');
      reportDbFailure();
      await producer.send({ topic: 'cart.add.result', messages: [{ key: correlationId, value: JSON.stringify({ correlationId, status: 'failure', reason: 'cart_db_error' }) }] });
    }
  } else {
    const reason = s.reason || (s.user === 'failure' ? 'user_failed' : 'product_failed');
    await producer.send({ topic: 'cart.add.result', messages: [{ key: correlationId, value: JSON.stringify({ correlationId, status: 'failure', reason }) }] });
