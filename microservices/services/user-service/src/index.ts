    ...
    await consumer.run({
        eachMessage: async ({ message }) => {
            ...
            try {
                ...
                const ok = !!user;
                const msg = ok
                    ? { correlationId, status: 'success', userId }
                    : { correlationId, status: 'failure', reason: 'user_not_found' };
                await producer.send({
                    topic: 'user.validation.result',
                    messages: [{ key: correlationId, value: JSON.stringify(msg) }],
                });
